...
set -x
export PATH=$PATH:/sbin:/usr/sbin:/bin:/usr/bin:/storage/local/sbin
RESOLVCONF=$(type -p resolvconf)
RESOLVCONF=/storage/local/sbin/resolvconf
VPNDNS=/storage/local/etc/VPNDNS.conf
NOVPNDNS=storage/local/etc/NOVPNDNS.conf
env > /storage/local/sbin/env.txt
case $script_type in

up)
   env | awk -F "=" '/foreign_option/ { print $2 }' | while read option; do
    echo $option
...
down)
  ...
  R=$(cat "$NOVPNDNS")
  echo -n "$R" | $RESOLVCONF -x -a "eth0.inet"
  ;;
esac
